...
const { deployConfigPath, deployConfigExt } = require('../utils/config-manager');
...
const downloadAndGenerate = templateUrl => {
    ...
    download(templateUrl, tmp, { clone: false }, err => {
        if (err) {
          ...
          // 根据 deployConfigExt 重命名文件
          const sourcePath = `${tmp}/deploy.config.js`;
          const targetPath = `${tmp}/deploy.config.${deployConfigExt}`;
          ...
          fs.renameSync(targetPath, deployConfigPath);
  
        spinner.stop();
        successLog('模板下载成功，模板位置：deploy/deploy.config.cjs');
